...
@section('title', $post_name->name)
@section('keywords', $post_name->keyword_seo ??'')
@section('description', $post_name->description_seo ??'')
@section('image', asset($post_name->avatar_path) ??'')
@section('schema', $post_name->schema_code ?? '')
@section('content')
        ...
        <div class="row">
            <div class="clm" style="--w-lg:12; --w-md:12; --w-xs:12">
                <div class="title_heading">
                    <h2>Theo ngành nghề<i class="fa fa-angle-double-right" aria-hidden="true"></i> <span>{{$post_name->name}}</span></h2>
                </div>
            </div>
            @if(isset($post_recu))
            @if(count($post_recu) == 0)
                <p>Đang cập nhật nội dung!</p>
            @else
                <div class="clm" style="--w-lg:12; --w-md:12; --w-xs:12">
                    <div class="row">
                        <div class="list_jobs">
                            @foreach ($post_recu as $item)
                            <div class="item">
                                <div class="box_job">
                                    ...
                                    <div class="group_logo_company">
                                        <div class="logo_company">
                                            @if(empty($item->company->avatar_path))
                                                <a href="{{ route('postRecruitmentDetail', ['slug' => $item->slug]) }}"
                                                    title="{{$item->name}}">
                                                    <img src="https://akm-img-a-in.tosshub.com/businesstoday/resource/market/company-logo/default-company-logo.png?size=40:40"
