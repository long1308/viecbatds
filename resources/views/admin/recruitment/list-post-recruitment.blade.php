...
<div class="content">
    ...
    @include('admin.partials.topbar', [
        'titleTab' => 'Quản lý tin tuyển dụng'
    ])
            ...
            <table class="table table-report -mt-2">
                <thead>
                    <tr>
                        ...
                        <th class="whitespace-nowrap">Tên tin tuyển dụng</th>
                ...
                <tbody>
                    ...
                    <tr class="intro-x">
                        ...
                        <td class="w-40">
                            <div class="flex">
                                <div class="w-10 h-10 image-fit" style="width: 5rem; height: 5rem;">
                                    ...
                                    @if (!$item->company_avatar)
                                        src="https://t4.ftcdn.net/jpg/04/73/25/49/360_F_473254957_bxG9yf4ly7OBO5I0O5KABlN930GwaMQz.jpg"
                                    @else
                                        src="{{ asset($item->company_avatar) }}"
                                    @endif 
                                    title="{{ $item->name }}">
                                </div>
                            </div>
                        ...
                        <td class="w-40">
                            @if ($item->active)
                            <div class="flex items-center justify-center text-success" style="cursor: pointer;" onclick="switchStatus($('#idHidden').val({{ $item->id }}))" data-tw-toggle="modal" data-tw-target="#hidden-post-recruitment-confirmation-modal"> <i data-lucide="eye" class="block mx-auto" style="margin: 5px;"></i> <p>Hiển thị</p> </div>
                            @else
                            <div class="flex items-center justify-center text-danger" style="cursor: pointer;" onclick="changeStatus({{ $item->id }}, 1)"> <i data-lucide="eye-off" class="block mx-auto" style="margin: 5px;"></i> <p>Ẩn</p> </div>
                            @endif
                        </td>
            ...
            <div class="modal-content">
                <div class="modal-body p-0">
                    <div class="p-5 text-center">
                        ...
                        <div class="text-3xl mt-5">Xóa tin tuyển dụng?</div>
            ...
            <div class="modal-content">
                <div class="modal-body p-0">
                    <div class="p-5 text-center">
                        ...
                        <div class="text-3xl mt-5">Bạn có chắc chắn muốn ẩn tin tuyển dụng này?</div>
...
function changeStatus(postId, status) {
    ...
    jQuery.ajax({
        url: `{{ route('admin.changeStatusPostRecruitment') }}`, 
        method: 'POST', 
        data: formData,
        contentType: false, 
        processData: false,
...
function changeHot(postId, hot) {
    ...
    jQuery.ajax({
        url: `{{ route('admin.changeHotPostRecruitment') }}`, 
        method: 'POST', 
        data: formData,
        contentType: false, 
        processData: false,
...
function deletePost(postIdDelete){
    ...
    jQuery.ajax({
        url: `{{ route('admin.deletePostRecruitment') }}`, 
        method: 'POST', 
        data: formData,
        contentType: false, 
        processData: false,
