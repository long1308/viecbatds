...
<div class="content">
    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            ...
            <div class="dropdown">
                <button id="submitData" class="dropdown-toggle btn btn-primary shadow-md flex items-center" aria-expanded="false" data-tw-toggle="dropdown" onclick="updateCode()"> Cập nhật </button>
            </div>
        </div>
    </div>
        ...
        <div class="intro-y col-span-12 lg:col-span-8">
            <div class="post intro-y overflow-hidden box mt-5">
                <ul class="post__tabs nav nav-tabs flex-col sm:flex-row bg-slate-200 dark:bg-darkmode-800" role="tablist">
                    <li class="nav-item">
                        <button title="Nội dung" onclick="switchTab(1)" data-tw-toggle="tab" data-tw-target="#content" class="nav-link tooltip w-full sm:w-40 py-4 active" id="content-tab" role="tab" aria-controls="content" aria-selected="true"> <i data-lucide="file-text" class="w-4 h-4 mr-2"></i> Mã code </button>
                    </li>
                </ul>
                ...
                <div class="post__content tab-content">
                    <div id="contentTab" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab">
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <input name="name" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập tên " value="{{ $code->name }}" onkeyup="renderData(this)">
                            </div>
                        </div>
                        <br>
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            <div class="font-medium flex items-center border-b border-slate-200/60 dark:border-darkmode-400 pb-5"> Mã code </div>
                            <div class="mt-5">
                                <textarea name="description" id="description" rows="5" style="width: 100%;">{{ $code->description }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-span-12 lg:col-span-4">
            <div class="intro-y box p-5">
                <div class="form-check form-switch flex flex-col items-start mt-3">
                    ...
                    <input id="post-form-6" name="active" class="form-check-input" type="checkbox" @if($code->active == 1) checked @endif>
                </div>
                <div class="form-switch flex flex-col items-start mt-3">
                    <label for="modal-form-2" class="form-label">Số thứ tự</label>
                    <input id="modal-form-2" name="order-code" type="number" class="form-control" placeholder="Nhập số thứ tự" value="{{ $code->order ?? 1 }}">
                </div>
            </div>
        </div>
        <!-- END: Post Info -->
...
@include('admin.partials.modal')
    <!-- Đặt mã JavaScript sau nút button -->
    <script>
        function updateCode(checkModal) {
            // validateRequire();
            ...
            jQuery.ajax({
                url: `{{ route('admin.updateCode', ['id' => $code->id]) }}`,
                method: 'POST',
                data: formData,
                contentType: false,
                processData: false,
