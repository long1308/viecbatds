    ...
    <!-- END: Top Bar -->
        <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
            ...
            <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                ...
                <div class="dropdown">
                    <button id="submitData" class="dropdown-toggle btn btn-primary shadow-md flex items-center" aria-expanded="false" data-tw-toggle="dropdown" onclick="updateContent()"> Cập nhật </button>
                </div>
            </div>
        </div>
                    ...
                    <div class="post__content tab-content">
                        <div id="contentTab" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab">
                            <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                                ...
                                <div class="mt-5">
                                    <input name="name" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập tên" value="{{ $content->name }}" onkeyup="renderData(this)">
                                </div>
                            </div>
                            <br>
                            <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                                <div class="font-medium flex items-center border-b border-slate-200/60 dark:border-darkmode-400 pb-5"> Đường dẫn </div>
                                <div class="mt-5">
                                    <input id="slug" name="slug" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập Đường dẫn" value="{{ $content->slug ?? "" }}">
                                </div>
                            </div>
                            <br>
                            <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                                ...
                                <div class="mt-5">
                                    <textarea name="description" id="description" rows="5" style="width: 100%;">{{ $content->description }}</textarea>
                                </div>
                            </div>
                            <br>
                            <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                                ...
                                <div class="mt-5">
                                    <textarea id="contentMain" name="content" class="form-control my-editor">{!! $content->content !!}</textarea>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div class="border-2 border-dashed dark:border-darkmode-400 rounded-md pt-4" style="width: 35%">
                                            <div class="flex flex-wrap px-4">
                                                <div class="w-24 h-24 relative image-fit mb-5 mr-5 cursor-pointer zoom-in">
                                                    ...
                                                    @else
                                                        <div id="imagePreview">
                                                            <img class="rounded-md" alt="Midone - HTML Admin Template" src="{{ asset($content->avatar_path)}}">
                                                        </div>
                        ...
                        <div id="seoTab" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab" style="display: none;">
                            <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                                ...
                                <div class="mt-5">
                                    <input id="titleSeo" name="titleSeo" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập title seo" value="{{ $content->title_seo }}">
                                </div>
                            </div>                            
                            <br>
                            <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                                ...
                                <div class="mt-5">
                                    <input id="keywordSeo" name="keywordSeo" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập từ khóa seo" value="{{ $content->keyword_seo }}">
                                </div>
                            </div>
                            <br>
                            <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                                ...
                                <div class="mt-5">
                                    <textarea name="descriptionSeo" id="descriptionSeo" style="width: 100%;" cols="30" rows="5" placeholder="Nhập mô tả seo">{{ $content->description_seo }}</textarea>
                                </div>
                            </div>
                            <br>
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <textarea name="schema" id="schema" style="width: 100%;" cols="30" rows="5" placeholder="Nhập mã schema">{{ $content->schema_code }}</textarea>
                            </div>
                        </div>
                        </div>
                    </div>
            ...
            <div class="col-span-12 lg:col-span-4">
                <div class="intro-y box p-5">
                        ...
                        <div class="mt-3">
                            ...
                            <div class="dropdown">
                                <div class="dropdown-toggle btn w-full btn-outline-secondary dark:bg-darkmode-800 dark:border-darkmode-800 flex items-center justify-start" role="button" aria-expanded="false" data-tw-toggle="dropdown">
                                    <div class="truncate">{{ $content->parent->name }}</div>
                                </div>
                            </div>
                        </div>
                    @endif
                    <div class="form-check form-switch flex flex-col items-start mt-3">
                        ...
                        <input id="post-form-5" name="hot" class="form-check-input hotInput" type="checkbox" @if($content->hot == 1) checked @endif>
                    </div>
                    <div class="form-check form-switch flex flex-col items-start mt-3">
                        <label for="post-form-6" class="form-check-label ml-0 mb-2">Hiển thị</label>
                        <input id="post-form-6" name="active" class="form-check-input" type="checkbox" @if($content->active == 1) checked @endif>
                    </div>
                    <div class="form-switch flex flex-col items-start mt-3">
                        <label for="modal-form-2" class="form-label">Số thứ tự</label>
                        <input id="modal-form-2" name="order-category" type="number" class="form-control" placeholder="Nhập số thứ tự" value="{{ $content->order ?? 1 }}">
                    </div>
                </div>
            </div>
            <!-- END: Post Info -->
        ...
        <script>
            ...
            function sendSubmit() {
                ...
                formData.append('parent_id', {{ $content->parent_id ?? 0 }});
                ...
                jQuery.ajax({
                    url: `{{ route('admin.updateContent', ['id' => $content->id]) }}`,
                    method: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            function redirectList(){
                window.location.href = '{{ route('admin.content') }}?parent_id={{ $content->parent_id ?? 0 }}';
                return;
            }
