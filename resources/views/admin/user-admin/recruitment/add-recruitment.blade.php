...
<div class="content">
    ...
    @if($type == config('constant.type-recruitment.category'))
        @include('admin.partials.topbar', [
            'titleTab' => 'Thêm mới ngành nghề tuyển dụng'
        ])
    @elseif($type == config('constant.type-recruitment.position'))
        @include('admin.partials.topbar', [
        'titleTab' => 'Thêm mới vị trí tuyển dụng'
        ])
    @endif
    <!-- END: Top Bar -->
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            @if($type == config('constant.type-recruitment.category'))
                Thêm mới ngành nghề tuyển dụng
            @elseif($type == config('constant.type-recruitment.position'))
        ...
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            ...
            <div class="dropdown">
                <button id="submitData" class="dropdown-toggle btn btn-primary shadow-md flex items-center" aria-expanded="false" data-tw-toggle="dropdown" onclick="submitRecruitment()"> Lưu </button>
            </div>
        </div>
    </div>
        ...
        <div class="col-span-12 lg:col-span-4">
            <div class="intro-y box p-5">
                ...
                <div class="form-switch flex flex-col items-start mt-3">
                    ...
                    <input id="modal-form-2" name="order" type="number" class="form-control" placeholder="Nhập số thứ tự" value="1">
                </div>
            </div>
        </div>
        <!-- END: Post Info -->
...
<script>
    function submitRecruitment(checkModal) {
        ...
        jQuery.ajax({
            url: '{{ route('admin.addRecruitment') }}',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
