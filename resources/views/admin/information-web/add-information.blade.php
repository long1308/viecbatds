...
<div class="content">
    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            ...
            <div class="dropdown">
                <button id="submitData" class="dropdown-toggle btn btn-primary shadow-md flex items-center" aria-expanded="false" data-tw-toggle="dropdown" onclick="submitSetting()"> Lưu </button>
            </div>
        </div>
    </div>
    <div class="pos intro-y grid grid-cols-12 gap-5 mt-5">
        ...
        <div class="intro-y col-span-12 lg:col-span-8">
            ...
            <div class="post intro-y overflow-hidden box mt-5">
                <ul class="post__tabs nav nav-tabs flex-col sm:flex-row bg-slate-200 dark:bg-darkmode-800" role="tablist">
                    ...
                    {{-- <li class="nav-item">
                        <button title="Adjust the meta title" onclick="switchTab(3)" data-tw-toggle="tab" data-tw-target="#meta-title" class="nav-link tooltip w-full sm:w-40 py-4" id="seo-tab" role="tab" aria-selected="false"> <i data-lucide="code" class="w-4 h-4 mr-2"></i> Seo </button>
                    </li> --}}
                </ul>
                <div class="post__content tab-content">
                    <div id="contentTab" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab">
                        ...
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <input id="link" name="link" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập link liên kết">
                            </div>
                        </div>
                        <br>
                    ...
                    <div id="seoTab" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab" style="display: none;">
                        ...
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <input id="descriptionSeo" name="descriptionSeo" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập mô tả seo">
                            </div>
                        </div>
                        <br>
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
        ...
        <div class="col-span-12 lg:col-span-4">
            <div class="intro-y box p-5">
                @if (isset($settingParent) && $settingParent->name)
                <div>
                    ...
                    <div class="dropdown">
                        <div class="dropdown-toggle btn w-full btn-outline-secondary dark:bg-darkmode-800 dark:border-darkmode-800 flex items-center justify-start" role="button" aria-expanded="false" data-tw-toggle="dropdown">
                            <div class="truncate">{{ $settingParent->name }}</div>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div class="form-switch flex flex-col items-start mt-3">
                    ...
                    <input id="modal-form-2" name="order-setting" type="number" class="form-control" placeholder="Nhập số thứ tự" value="1">
                </div>
            </div>
        </div>
        <!-- END: Post Info -->
...
<script>
    function submitSetting(checkModal) {
        ...
        formData.append('order', $('input[name="order-setting"]').val());
        formData.append('description', $('#description').val());
        formData.append('content', content);
        formData.append('parent_id', {{ $settingParent->id ?? 0 }});
        formData.append('type', "{{ $settingParent->type }}");
        ...
        jQuery.ajax({
            url: '{{ route('admin.createInformation') }}',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
                ...
                window.location.href = '{{ route('admin.information', ['type' => $settingParent->type]) }}?parent_id={{ $settingParent->id ?? 0 }}';
            },
