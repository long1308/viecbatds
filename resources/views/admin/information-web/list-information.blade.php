...
<div class="content">
        <!-- BEGIN: Top Bar -->
    @if($type == config('constant.type-setting.header'))
        @include('admin.partials.topbar', [
            'titleTab' => 'Quản lý header'
        ])
    @elseif($type == config('constant.type-setting.body'))
        @include('admin.partials.topbar', [
        'titleTab' => 'Quản lý trang chủ'
        ])
    @elseif($type == config('constant.type-setting.footer'))
        @include('admin.partials.topbar', [
        'titleTab' => 'Quản lý footer'
        ])
    @endif
    ...
    <!-- END: Top Bar -->
        ...
        <div class="grid grid-cols-12 gap-6 mt-5">
            <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                ...
                @checkPermission('create-information-web')
                <a href="{{ route('admin.addInformation') }}?parent_id={{ request('parent_id') ?? 0 }}"><button class="btn btn-primary shadow-md mr-2">Thêm mới <span class="w-5 h-5 flex items-center justify-center"> <i class="w-4 h-4" data-lucide="plus"></i> </span></button></a>
                @endcheckPermission
            ...
            <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
                <table class="table table-report -mt-2">
                    ...
                    @if(count($listInformation)>0)
                        @foreach($listInformation as $item)
                        <tr class="intro-x">
                            <td>
                                <i @if(count($item->childs)>0) data-lucide="folder" @else data-lucide="file-minus" @endif class="block mx-auto" style="margin: 0px;"></i>
                            </td>
                            <td>
                                <a href="{{ route('admin.information', ['type' => $item->type]) }}?parent_id={{ $item->id }}" class="font-medium whitespace-nowrap">{{ $item->name }}</a>
                            </td>
                            <td style="text-align: center;">
                                <div style="display: flex; justify-content: center;">
                                    <input type="number" value="{{ $item->order }}" onblur="updateOrderInformation({{  $item->id }}, this)" onkeypress="handleKeyPress(event, {{  $item->id }}, this)" style="width: 55px;">
                                </div>
                            </td>
                            <td class="w-40">
                                @if($item->active == 1)
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @checkPermission('create-information-web')
                                    <a class="flex items-center mr-3 text-primary" href="{{ route('admin.addInformation') }}?parent_id={{ $item->id ?? 0 }}" data-tw-toggle="modal" data-tw-target="#add-modal"> <i data-lucide="plus" class="block mx-auto"></i>  Thêm </a>
                                    @endcheckPermission
                                    @checkPermission('edit-information-web')
                                    <a class="flex items-center mr-3" href="{{ route('admin.editInformation', ['id' => $item->id]) }}" data-tw-toggle="modal" data-tw-target="#edit-modal"> <i data-lucide="check-square" class="w-4 h-4 mr-1"></i> Sửa </a>
                                    @endcheckPermission
                                    @checkPermission('delete-information-web')
                                    @if ($item->id == 4 || $item->id == 5 || $item->id == 6)
                                        
                                    @else
                                    <a class="flex items-center text-danger" href="javascript:;" data-tw-toggle="modal" data-tw-target="#delete-confirmation-modal" onclick="settingId({{ $item->id }})"> <i data-lucide="trash-2" class="w-4 h-4 mr-1"></i> Xóa </a>
                                    @endif
                                    @endcheckPermission
                                </div>
                            </td>
                ...
                <div class="modal-content">
                    <div class="modal-body p-0">
                        <div class="p-5 text-center">
                            ...
                            <input type="hidden" name="idSetting" value="">
                        </div>
                        <div class="px-5 pb-8 text-center">
                            <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-24 mr-1">Hủy</button>
                            <button type="button" data-tw-dismiss="modal" class="btn btn-danger w-24" onclick="deleteSetting()">Xóa</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    function settingId(itemId) {
        $('input[name="idSetting"]').val(itemId);
    }
    function deleteSetting(button) {
        var idSetting = $('input[name="idSetting"]').val();
        var formData = new FormData();
        formData.append('_token', '{{ csrf_token() }}');
        formData.append('id', idSetting);

        jQuery.ajax({
            url: '{{ route('admin.deleteInformation') }}',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    function redirectList(){
        window.location.href = '{{ route('admin.information', ['type' => $type]) }}?parent_id={{ request('parent_id') }}';
        return;
    }

    function handleKeyPress(event, itemId, inputElement) {
            if (event.keyCode === 13) {
                updateOrderInformation(itemId, inputElement);
            }
        }

    function updateOrderInformation(informationId, orderInput) {
        ...
        jQuery.ajax({
            url: `{{ route('admin.updateOrderInformation') }}`, 
            method: 'POST', 
            data: formData,
            contentType: false, 
            processData: false,
