...
<div class="content">
    ...
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        ...
        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
            ...
            <div class="dropdown">
                <button id="submitData" class="dropdown-toggle btn btn-primary shadow-md flex items-center" aria-expanded="false" data-tw-toggle="dropdown" onclick="updateSetting()"> Cập nhật </button>
            </div>
        </div>
    </div>
    <div class="pos intro-y grid grid-cols-12 gap-5 mt-5">
                ...
                <div class="post__content tab-content">
                    <div id="contentTab" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab">
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <input name="name" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập tên " value="{{ $setting->name }}" onkeyup="renderData(this)">
                            </div>
                        </div>
                        <br>
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            <div class="font-medium flex items-center border-b border-slate-200/60 dark:border-darkmode-400 pb-5"> Link liên kết </div>
                            <div class="mt-5">
                                <input id="link" name="link" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập link liên kết" value="{{ $setting->link ?? "" }}">
                            </div>
                        </div>
                        <br>
                                    ...
                                    <div class="border-2 border-dashed dark:border-darkmode-400 rounded-md pt-4" style="width: 35%">
                                        <div class="flex flex-wrap px-4">
                                            <div class="w-24 h-24 relative image-fit mb-5 mr-5 cursor-pointer zoom-in">
                                                ...
                                                @else
                                                <div id="imagePreview">
                                                    <img class="rounded-md" alt="Midone - HTML Admin Template" src="{{ asset($setting->avatar_path)}}">
                                                </div>
                        ...
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <textarea name="description" id="description" rows="5" style="width: 100%;">{{ $setting->description }}</textarea>
                            </div>
                        </div>
                        <br>
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <textarea id="contentMain" name="content" class="form-control my-editor">{!! $setting->content !!}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="seoTab" class="tab-pane p-5 active" role="tabpanel" aria-labelledby="content-tab" style="display: none;">
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <input id="titleSeo" name="titleSeo" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập title seo" value="{{ $setting->title_seo }}">
                            </div>
                        </div>
                        <br>
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <input id="descriptionSeo" name="descriptionSeo" type="text" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập mô tả seo" value="{{ $setting->description_seo }}">
                            </div>
                        </div>
                        <br>
                        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
                            ...
                            <div class="mt-5">
                                <input id="keywordSeo" name="keywordSeo" class="intro-y form-control py-3 px-4 box pr-10" placeholder="Nhập từ khóa seo" value="{{ $setting->keyword_seo }}">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-span-12 lg:col-span-4">
            <div class="intro-y box p-5">
                ...
                @endif
                <div class="form-check form-switch flex flex-col items-start mt-3" style="display: none;">
                    <label for="post-form-5" class="form-check-label ml-0 mb-2">Nổi bật</label>
                    <input id="post-form-5" name="hot" class="form-check-input hotInput" type="checkbox" @if($setting->hot == 1) checked @endif>
                </div>
                <div class="form-check form-switch flex flex-col items-start mt-3">
                    ...
                    <input id="post-form-6" name="active" class="form-check-input" type="checkbox" @if($setting->active == 1) checked @endif>
                </div>
                <div class="form-switch flex flex-col items-start mt-3">
                    <label for="modal-form-2" class="form-label">Số thứ tự</label>
                    <input id="modal-form-2" name="order-category" type="number" class="form-control" placeholder="Nhập số thứ tự" value="{{ $setting->order ?? 1 }}">
                </div>
            </div>
        </div>
        <!-- END: Post Info -->
...
<script>
    function updateSetting(checkModal) {
        ...
        formData.append('parent_id', {{ $setting->parent_id ?? 0 }});
        ...
        jQuery.ajax({
            url: `{{ route('admin.updateInformation', ['id' => $setting->id]) }}`,
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
                ...
                window.location.href = '{{ route('admin.information', ['type' => $setting->type]) }}?parent_id={{ $setting->parent_id ?? 0 }}';
            },
